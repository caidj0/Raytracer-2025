...
impl Sphere {
    ...
    fn random_to_sphere(radius: f64, distance_squared: f64) -> Vec3 {
        let r1 = Random::f64();
        let r2 = Random::f64();
        let y = 1.0 + r2 * ((1.0 - radius * radius / distance_squared).sqrt() - 1.0);

        let phi = 2.0 * PI * r1;
        let x = phi.cos() * (1.0 - y * y).sqrt();
        let z = phi.sin() * (1.0 - y * y).sqrt();
...
impl Hittable for Sphere {
    ...
    fn random(&self, origin: &Point3) -> UnitVec3 {
        ...
        UnitVec3::from_vec3(uvw.onb_to_world(Self::random_to_sphere(self.radius, distance_squared)))
            .unwrap()
    }
}
